...
import ru.gb.market.back.mapper.ProductsMapper;
...
import ru.gb.market.back.exceptions.DataValidationException;
...
//        return productService.findAll(pageIndex-1,pageSize).map(ProductDto::new);
        return productService.findAll(pageIndex-1,pageSize).map(mapper::toDto);
...
//  //              .get());
        return mapper.toDto(productService.findProductById(id).orElseThrow(() -> new ResourceNotFoundException("Product id = " + id + " not found")));
    }
